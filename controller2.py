    ...
    def act_like_router(self, frame, packet_in):
        if frame.type == 0x0806:
            ...
            if packet.opcode == 1 and packet.protodst in self.routerip:  # arp request and ip dst in routing table
                arp_data = arp(hwtype=packet.hwtype, prototype=packet.prototype, hwlen=packet.hwlen, protolen=packet.protolen, opcode=2,
            ...
            if network == 0:
                ...
                if data_icmp.type == 8 and packet.dstip in self.routerip:
                    ...
                    self.store = frame
                    arp_type = arp(hwlen=6, hwdst=ETHER_BROADCAST, protodst=packet.dstip,
                                   hwsrc=addresses.EthAddr('FA:DE:DD:ED:AF:AA'), protosrc=packet.srcip)
                    arp_type.opcode = 1
